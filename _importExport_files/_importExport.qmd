...
The third one is importing data from other software.
...
We will see the specific example in the part of importing Execl files.
...
‌Format,feather is a binary file format for storing and sharing data. 
...
df_feather = pd.read_feather("nyccrashes_cleaned.feather")
...
+ We can check whether the corresponding csv file is generated in the working directory.
...
#### Excel files
We want to import Excel files but there are not suitable Excel files.
We can transfer the CSV file to an Excel file by the fucntion `to_excel` from `Pandas`.
...
+ Check whether the corresponding Excel file is generated in the working directory

Now we have generated the Excel file covering the same data. 
And then we can learn how to import Excel files.
The function wen use is `read_csv` no matter what `Pandas` version is.
...
We are able to generate the TXT file in the similar way as generatng the Excel file. 
...
#### SAS files
##### Transfer the CSV files to a `.xpt` file
