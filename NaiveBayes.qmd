...
- We want to predict whether a crash will be severe based on our independent X predictors: 
    - Borough, crash time, contributing factor vehicle 1, contributing factor vehicle 2.
...
# Select predictors.
predictors = ['BOROUGH', 'CRASH TIME', 'CONTRIBUTING FACTOR VEHICLE 1', 'CONTRIBUTING FACTOR VEHICLE 2']
...
<ul style="font-size: 25px;">
  <li>True Negatives (TN): 276 (correctly predicted as class 0)
  <li>False Positives (FP): 226 (incorrectly predicted as class 1)</li>
  <li>False Negatives (FN): 26 (incorrectly predicted as class 0)</li>
  <li>True Positives (TP): 35 (correctly predicted as class 1)</li>
</ul>
<p style="font-size: 25px;"></p>
<ul style="font-size: 25px;">
 <li> We can say that the model is relatively good at predicting non-severe crashes with 91% precision and moderate recall of 55%, but it performs poorly in identifying severe crashes with 0.13 precision (very low precision).</li>
 <li> Our original model accuracy is 0.55, but the mean cross-validation accuracy is 0.53. So, the 10-fold cross-validation accuracy test does not result in performance improvement for this model.</li>
 <li> Based on this model, approximately 53% of crashes are predicted to be not severe.</li>
 </ul>
...
<p style="font-size: 25px;"></p>
<ul style="font-size: 25px;">
 <li> An AUC (Area Under the Curve) of 0.56 indicates that the model has a slight ability to distinguish between the two classes (severe crash vs. not severe crash), but it is not performing well. This might be due to the fact that Naive Bayes does not capture non-linear relationships effectively. </li>
 </ul>
...
- The overall accuracy of 53% and AUC of 56% indicates that the model's performance could be improved.
