...
# Function to create a lambda grid
def compute_lambda_max_logistic(X, y):
    """
    Compute lambda_max for logistic regression (smallest lambda at which all coefficients are zero).
    ...
    Returns:
    - lambda_max: The largest lambda for which all coefficients are zero
    """
    # Standardize the features
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    
    # Initial prediction probability (0.5 since all coefficients are zero)
    p = 0.5 * np.ones_like(y)
    
    # Calculate gradient at zero coefficients
    gradient = np.dot(X_scaled.T, p - y)
    
    # Lambda max is the maximum of the absolute gradients divided by the number of samples
    lambda_max = np.max(np.abs(gradient)) / X_scaled.shape[0]
    
    return lambda_max


def create_lambda_grid(X, y, lambda_min_ratio, num_lambdas=100):
    """
    Create a logarithmically spaced lambda grid for L1-penalized logistic regression.
    
    Parameters:
    - X: Feature matrix (numpy array or similar)
    - y: Target vector (numpy array or similar)
    - lambda_min_ratio: The ratio of the smallest lambda in the grid to lambda_max
    - num_lambdas: Number of lambda values to generate in the grid
    
    Returns:
    - lambdas: Array of lambda values from lambda_max to lambda_min, logarithmically spaced
    - Cs: Array of C values for use in scikit-learn, where C = 1/lambda
    """
    # Compute lambda_max for logistic regression
    lambda_max = compute_lambda_max_logistic(X, y)
    
    # Compute lambda_min using the lambda_min_ratio
    lambda_min = lambda_max * lambda_min_ratio
    
    # Generate a sequence of lambdas from lambda_max to lambda_min, logarithmically spaced
    lambdas = np.logspace(np.log10(lambda_max), np.log10(lambda_min), num=num_lambdas)
    
    # Convert lambdas to C values for use in scikit-learn (C = 1/lambda)
    Cs = 1 / lambdas
...
# Generate lambda and C grid
lambda_min_ratio = 0.01
lambdas, Cs = create_lambda_grid(X, y, lambda_min_ratio)

# Fit logistic regression model for each C and store coefficients
coefficients = []
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)
...
# Plot solution path against C (inverse of lambda)
coefficients = np.array(coefficients)
plt.figure(figsize=(10, 6))

for i in range(coefficients.shape[1]):
    plt.plot(Cs, coefficients[:, i], label=f'Feature {i+1}')

plt.xscale('log')
plt.xlabel('C (Inverse of Lambda)')
plt.ylabel('Coefficients')
plt.title('Solution Path of Lasso Logistic Regression')
plt.legend()
plt.show()
```
